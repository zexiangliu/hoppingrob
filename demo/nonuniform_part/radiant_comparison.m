...
s_part = Partition(Rec([20 28; 20 28; 20 28]));
s_part.add_area(goal_set);
s_part.check();
...
if dmax
  ...
  fx1 = a1 * xvar + k1 + e1 * dvar;
  fx2 = a2 * xvar + k2 + e2 * dvar;
  s_part.abstract({fx1, fx2}, [xvar; dvar], d_rec, system_setting, encoding_setting);
else
  fx1 = a1 * xvar + k1;
  fx2 = a2 * xvar + k2;
  s_part.abstract({fx1, fx2}, [xvar], [], TransSyst.sparse_set);
end

% Search for transient regions
part.search_trans_reg(pg_depth);

s_part.search_trans_reg(pg_depth);

%%%%%%%%%%%%%%%%%%%%%%%%
% SYNTHESIS-REFINEMENT %
%%%%%%%%%%%%%%%%%%%%%%%%

eq = @(A, B) length(A) == length(B) && all(A(:) == B(:));
...
while true
  ...
  s_B = s_part.get_cells_with_ap({'SET'});
...
%   end
  ...
  [s_Win, s_Cwin] = s_part.ts.win_primal([], s_B, [], 'exists', 'forall', s_Win);
  s_Cwin = setdiff(s_Cwin, s_Win);
...
%   Split largest cell in candidate set
  ...
  [~, s_C_index] = max(volume(s_part.cell_list(s_Cwin)));
  s_part.split_cell(s_Cwin(s_C_index));
