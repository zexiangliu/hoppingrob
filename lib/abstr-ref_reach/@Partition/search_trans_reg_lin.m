function [] = search_trans_reg_lin(part, search_depth)
...
function bool = is_transient(search_set, dyn_list, d_rec)
    bool = true;
    num_U = length(dyn_list);
    for i = 1:num_U
        eq = dyn_list{i}{1}\(dyn_list{i}{2}+dyn_list{i}{3});
        if(any(eq>=search_set.xmin' & eq<=search_set.xmax'))
            bool = false;
            return;
        end
    end
