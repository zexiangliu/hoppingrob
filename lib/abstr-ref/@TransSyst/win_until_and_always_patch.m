function [V, Cv, cont] = win_until_and_always_patch(ts, A, B, P, quant1)
...
%       [Vinv, Cvinv, cont_inv] = ts.win_intermediate(uint32(1:ts.n_s), A, [], {uint32(1:ts.n_s)}, quant1);
    ...
    elseif nargout > 1
      ...
      [V, Cvu] = ts.win_until_patch(intersect(B, Vinv), intersect(P, Vinv), quant1);
      Cv = union(Cvinv, Cvu);
    else
      Vinv = ts.win_intermediate(uint32(1:ts.n_s), A, [], {uint32(1:ts.n_s)}, quant1);
      V = ts.win_until_patch(intersect(B, Vinv), intersect(P, Vinv), quant1);
    end
  else
    % No need to worry about []A
    if nargout > 2
      [V, Cv, cont] = ts.win_until_patch(B, P, quant1);
      cont.from = 'win_until';
    elseif nargout > 1
      [V, Cv] = ts.win_until_patch(B, P, quant1);
    else
      V = ts.win_until_patch(B, P, quant1);
    end
  end
end
