function [V, Cv, cont] = win_intermediate_patch(ts, A, B, P, C_list, quant1)
  ...
  while true
    ...
    for i=1:length(C_list)
      ...
      if nargout > 2
        [Vti, Cvi, Ki] = ts.win_until_and_always_patch(A, B, Qi, quant1);
        Klist{i} = Ki;
      elseif nargout > 1
        [Vti, Cvi] = ts.win_until_and_always_patch(A, B, Qi, quant1);
        Cv = union(Cv, Cvi);
      else
        Vti = ts.win_until_and_always_patch(A, B, Qi, quant1);
      end
      Vt = intersect(Vt, Vti);
    end
